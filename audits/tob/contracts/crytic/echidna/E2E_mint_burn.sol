...
import '../../../../../contracts/conanswapV3Pool.sol';
...
contract E2E_mint_burn {
    SetupTokens tokens;
    Setupconanswap conanswap;

    conanswapV3Pool pool;

    TestERC20 token0;
    TestERC20 token1;

    conanswapMinter minter;
    conanswapSwapper swapper;
    ...
    constructor() public {
        ...
        conanswap = new Setupconanswap(token0, token1);

        minter = new conanswapMinter(token0, token1);
        swapper = new conanswapSwapper(token0, token1);
    ...
    function check_mint_invariants(
        int24 _tickLower,
        int24 _tickUpper,
        conanswapMinter.MinterStats memory bfre,
        conanswapMinter.MinterStats memory aftr
    ) internal {
        (, int24 currentTick, , , , , ) = pool.slot0();
    ...
    function _init(uint128 _seed) internal {
        ...
        // deploy the pool
        //
        conanswap.createPool(poolParams.fee, poolParams.startPrice);
        pool = conanswap.pool();
    ...
    function test_mint(uint128 _amount) public {
        ...
        (conanswapMinter.MinterStats memory bfre, conanswapMinter.MinterStats memory aftr) =
    ...
    function test_burn_partial(uint128 _amount) public {
        ...
        try minter.doBurn(pos.tickLower, pos.tickUpper, burnAmount) returns (
            conanswapMinter.MinterStats memory bfre_burn,
            conanswapMinter.MinterStats memory aftr_burn
        ) {
            bfre = bfre_burn;
            aftr = aftr_burn;
        } catch {
